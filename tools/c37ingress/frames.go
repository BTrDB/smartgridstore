...
//
// func ReadFrame(r io.Reader) (*CommonHeader, Frame, error) {
...
// 		fmt.Printf("frame checksum failure type=%d, got=%d expected=%d\n", ch.SyncType(), realchk, expectedchk)
// 		return fmt.Errorf("Frame checksum failure\n")
// 	}
...
func (d *DataFrame) PrettyDump() {
	fmt.Printf("SOC: %d\n", d.SOC)
	fmt.Printf("FRAC: %d/%d\n", d.FRACSEC, d.TIMEBASE)
	fmt.Printf("REAL NANOS: %d\n", d.UTCUnixNanos)
	fmt.Printf("TIMESTAMP: %s\n", time.Unix(0, d.UTCUnixNanos))
	for i := 0; i < len(d.Data); i++ {
		el := d.Data[i]
		fmt.Printf("PMU %d (%s)\n", i, el.STN)
		fmt.Printf(" FREQ  %f Hz\n", el.FREQ)
		fmt.Printf(" DFREQ %f Hz\n", el.DFREQ)
		fmt.Printf(" STAT  %d \n", el.STAT)
		for phi, nm := range el.PHASOR_NAMES {
			unit := "Volts"
			if !el.PHASOR_ISVOLT[phi] {
				unit = "Amperes"
			}
			fmt.Printf(" PH[%d] %16s : MAG=%f ANG=%f %s\n", phi, nm,
				el.PHASOR_MAG[phi],
				el.PHASOR_ANG[phi],
				unit)
		}
		for ani, nm := range el.ANALOG_NAMES {
			fmt.Printf(" AN[%d] %16s : %f\n", i, nm, el.ANALOG[ani])
		}
		for dgi, nm := range el.DIGITAL_NAMES {
			fmt.Printf(" DG[%d] %16s : %x\n", i, nm, el.DIGITAL[dgi])
		}
	}
}
